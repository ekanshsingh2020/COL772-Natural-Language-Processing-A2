...
for epoch in range(350):
    ...
    k=0
    for i in range(0, len(batched_data_train), 5001):
        k+=1
        batch = batched_data_train[i:i+5001]
        # print(len(batch))
        batch_ques_train = []
        batch_col_train = []
        batch_lab_train = []
    ...
    with torch.no_grad():
        acc_test = 0
        for i in range(len(batched_data_test)):
            batch_ques_test = batched_data_test[i][0].unsqueeze(0).to(device)
            batch_col_test = batched_data_test[i][1].unsqueeze(0).to(device)
            batch_lab_test = batched_data_test[i][2].unsqueeze(0).to(device)
        ...
        print(f'Accuracy on test set: {acc_test/len(batched_data_test)}')
